...
export const pizzaSlice = createSlice({
  ...
  reducers: {
    ...
    addToPizza: (state, action: PayloadAction<IFlavors>) => {
      ...
      const exists = state.pizza.flavors?.find(
        (flavor) => flavor.id === action.payload.id
      );
      if (exists) {
      ...
      state.pizza.name = generatePizzaName(
        state.pizza.size,
        state.pizza.flavors ?? []
      );
      state.pizza.price = updatePizzaPrice(
        state.pizza.size,
        state.pizza.flavors ?? []
      );
    },
    removeFromPizza: (state, action: PayloadAction<number>) => {
      state.pizza.flavors = state.pizza.flavors?.filter(
        (flavor) => flavor.id !== action.payload
      );
      state.pizza.name = generatePizzaName(
        state.pizza.size,
        state.pizza.flavors ?? []
...
export const { reducer: pizzaSliceReducer, actions: pizzaSliceActions } =
  pizzaSlice;
