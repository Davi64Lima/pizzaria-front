...
export default function TrackerPage() {
  ...
  return (
    <section className="flex items-center justify-center min-h-[calc(100vh-64px)] bg-gray-50 p-4">
      {" "}
      {/* Ajuste o min-h para sua altura de header/footer se tiver */}
              ...
              {foundOrder && (
                <div className="mt-4 border-t border-blue-200 pt-4">
                  <h3 className="text-md font-bold mb-2">
                    Detalhes do Pedido #{foundOrder.code}:
                  </h3>
                  <ul>
                    {foundOrder.products &&
                  ...
                  {foundOrder.value && (
                    <p className="mt-2 font-semibold">
                      Total: R$ {(foundOrder.value / 100).toFixed(2)}
                    </p>
                  )}
                </div>
              )}
